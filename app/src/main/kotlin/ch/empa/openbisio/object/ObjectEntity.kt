...
data class ObjectEntity(
    ...
    val autoGeneratedCode: Boolean = false
    //val children: List<ConcreteIdentifier.SampleIdentifier> = listOf(),
    //val parents: List<ConcreteIdentifier.SampleIdentifier> = listOf()
) : HierarchicalEntity, CreatableEntity {


    override fun persist(): List<CreateSamplesOperation> {
        val sc = SampleCreation().apply {
            this.code = if (!autoGeneratedCode) identifier.code else null
            this.experimentId = ExperimentIdentifier(identifier.getAncestor().identifier)
            this.spaceId = SpacePermId(identifier.space().identifier)
            this.projectId = ProjectIdentifier(identifier.project().identifier)
            this.typeId = EntityTypePermId(type)
            properties = this@ObjectEntity.properties
            isAutoGeneratedCode = autoGeneratedCode
        }
    ...
    override fun children(): Collection<HierarchicalEntity> {
        return listOf()
    }
